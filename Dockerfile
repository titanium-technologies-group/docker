...
RUN apk --no-cache add py-pip curl openssh-client git jq py2-dateutil py2-magic bash && \
    ...
    printf "#!/bin/sh\ndocker run -v \$PWD:/app tico/php-cs-fixer" > /usr/local/bin/php-cs-fixer && \
    printf "#!/bin/sh\ndocker run -v \$PWD:/app tico/security-checker" > /usr/local/bin/security-checker && \
    printf "#!/bin/sh\ndocker run -v \$PWD:/app tico/unused-scanner" > /usr/local/bin/unused-scanner && \
