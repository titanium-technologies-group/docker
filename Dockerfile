...
RUN apk --no-cache add py-pip curl openssh-client && \
    ...
    printf "#!/bin/sh\nexec docker run -v $(pwd):/app rakshazi/php-cs-fixer \$@" > /usr/local/bin/php-cs-fixer && \
    printf "#!/bin/sh\nexec docker run -i -v $(pwd):/app composer update --no-dev --ignore-platform-reqs --no-interaction --no-progress -a -d /app" > /usr/local/bin/composer-update && \
